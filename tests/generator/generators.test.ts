...
const mockConfig: AirulesConfig = {
	...
	rules: {
		style: { prefer_functional: true, naming_convention: "camelCase", import_style: "named", quote_style: "double" },
		architecture: { pattern: "feature-based", api_style: "server-actions", state_management: "zustand" },
		testing: { framework: "vitest", style: "arrange-act-assert", min_coverage: 80, require_tests_for: ["utils"] },
		git: { commit_style: "conventional", branch_pattern: "feature/*", require_pr: true },
		docs: { require_jsdoc: false, language: "english" },
		security: { no_secrets_in_code: true, sanitize_inputs: true, prefer_parameterized_queries: true },
